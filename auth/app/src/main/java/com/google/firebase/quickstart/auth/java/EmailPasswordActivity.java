...
public class EmailPasswordActivity extends BaseActivity implements
    ...
    private static final String TAG = "Почта и Пароль";
    ...
    private void createAccount(String email, String password) {
        Log.d(TAG, "Создать аккаунт: " + email);
        if (!validateForm()) {
            return;
        }
                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            Log.w(TAG, "Пользователь не создан, ошибка!", task.getException());
                            Toast.makeText(EmailPasswordActivity.this, "Ошибка аутентификации.",
        ...
        mAuth.signInWithEmailAndPassword(email, password)
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            // Sign in success, update UI with the signed-in user's information
                            Log.d(TAG, "Вход с помощью почты: Успех");
                            FirebaseUser user = mAuth.getCurrentUser();
                            updateUI(user);
                        } else {
                            // If sign in fails, display a message to the user.
                            Log.w(TAG, "Вход с помощью почты: Ошибка", task.getException());
                            Toast.makeText(EmailPasswordActivity.this, "Ошибка аутентификации.",
                                    Toast.LENGTH_SHORT).show();
                            updateUI(null);
                            // [START_EXCLUDE]
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        ...
                        if (task.isSuccessful()) {
                            Toast.makeText(EmailPasswordActivity.this,
                                    "Проверочное письмо отправлено " + user.getEmail(),
                                    Toast.LENGTH_SHORT).show();
                        } else {
                            Log.e(TAG, "Отправка письма", task.getException());
                            Toast.makeText(EmailPasswordActivity.this,
                                    "Не удалось отправить проверочное письмо.",
                                    Toast.LENGTH_SHORT).show();
                        }
                        // [END_EXCLUDE]
                    }
